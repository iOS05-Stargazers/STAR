...
class ShieldConfigurationExtension: ShieldConfigurationDataSource {
    ...
    /// 앱/도메인 웹 사용 제한 중일 경우 overlay되는 ShieldView는 ShieldConfiguration를 통해 커스터마이징 할 수 있음
    /// 각 configuration 메서드로부터 전달받은 앱 정보를 통해 커스터마이징 가능
    ///  * ShieldConfiguration init 이외에 ShieldView의 커스터마이징은 제한됨
    ///  * 더 많은 정보는 아래 링크를 확인:
    ...
    private func setShieldConfig(
        ...
        let CUSTOM_PRIMARY_BUTTON_LABEL = ShieldConfiguration.Label(
            ...
            let CUSTOM_PRIAMRY_BUTTON_BACKGROUND: UIColor = .starAccent
